...
class TestMemory(unittest.TestCase):
    ...
    def test_find_max_related_memory_success(self):
        mem1 = self.data.add_memory()
        mem2 = self.data.add_memory({constants.PARENT_MEM: [5, 6, mem1[constants.MID]]})
        mem3 = self.data.add_memory({constants.PARENT_MEM: [6, mem1[constants.MID], 8]})
        seq_time_memories = [mem2, mem3]
        memories = memory.find_update_max_related_memories(seq_time_memories)
        self.assertEqual(1, len(memories))
        mem = self.data._get_memory(mem2[constants.MID])
        self.assertEqual(mem1[constants.MID], mem[constants.PARENT_MEM][0])

    def test_find_max_related_memory_empty(self):
        mem1 = self.data.add_memory({constants.PARENT_MEM: [5, 6, 7]})
        mem2 = self.data.add_memory({constants.PARENT_MEM: [6, 7, 8]})
    ...
    def test_find_max_related_memory_limit(self):
        mem1 = self.data.add_memory()
        mem2 = self.data.add_memory()
        mem3 = self.data.add_memory({constants.PARENT_MEM: [5, 6, mem1[constants.MID], mem2[constants.MID]]})
        mem4 = self.data.add_memory({constants.PARENT_MEM: [6, mem1[constants.MID], mem2[constants.MID], 8]})
    ...
    def get_live_sub_memories_success(self):
        mem1 = self.data.add_memory()
        mem2 = self.data.add_memory()
        mem3 = self.data.add_memory({memory.CHILD_MEM: [mem1[constants.MID], mem2[constants.MID]]})
        child_memories1 = memory.get_live_sub_memories(mem3, memory.CHILD_MEM)
        self.assertEqual(2, len(child_memories1))

        mem4 = self.data.add_memory({memory.CHILD_MEM: [mem1[constants.MID], mem2[constants.MID], 9]})
        ...
        el5 = self.data.add_memory({memory.CHILD_MEM: [mem1[constants.MID], 9, mem2[constants.MID]]})
        ...
        el6 = self.data.add_memory({memory.CHILD_MEM: [8, 9]})
        memory.get_live_sub_memories(el6, memory.CHILD_MEM)
        mem = self.data._get_memory(el6[constants.MID])
        self.assertIsNone(mem)

    def test_get_memories(self):
        mem1 = self.data.add_memory({constants.STATUS: constants.MATCHED, constants.DURATION: 1, constants.REWARD: 0.5})
        mem2 = self.data.add_memory({constants.STATUS: constants.MATCHING, constants.DURATION: 1, constants.REWARD: 0.6})
        mem3 = self.data.add_memory({constants.STATUS: constants.EXPIRED, constants.DURATION: 3, constants.REWARD: 0.3})
    ...
    def test_find_max_related_memories(self):
        ...
        mem5 = self.data.add_memory({constants.PARENT_MEM: [mem1[constants.MID], mem2[constants.MID], mem3[constants.MID]]})
        mem6 = self.data.add_memory({constants.PARENT_MEM: [mem2[constants.MID], mem3[constants.MID], mem4[constants.MID]]})
        memories = [mem5, mem6]
        ...
        mem7 = self.data.add_memory({constants.PARENT_MEM: [mem2[constants.MID], mem3[constants.MID], mem4[constants.MID]]})
    ...
    def test_remove_dead_memories(self):
        child_mem = ['a1', 'b2', 'c3']
        mem1 = self.data.add_memory({memory.CHILD_MEM: child_mem})
        forget_ids = ['c3']
        memory.remove_dead_memories(memory.CHILD_MEM, child_mem, forget_ids, mem1[constants.MID])
        mem1b = self.data._get_memory(mem1[constants.MID])
        forget_ids2 = ['a1', 'b2']
        ...
        mem1c = self.data._get_memory(mem1[constants.MID])
    ...
    def test_search_sub_memories(self):
        ...
        mem2 = self.data.add_memory()
        mem3 = self.data.add_memory({memory.CHILD_MEM: [mem1[constants.MID], mem2[constants.MID]]})
        mem4 = self.data.add_memory()
        mem6 = self.data.add_memory({memory.CHILD_MEM: [mem4[constants.MID], mem2[constants.MID]]})
        memories = [mem3, mem6]
    ...
    def test_create_working_memory(self):
        ...
        mem1 = self.data.add_memory({constants.REWARD: 1})
        mem2 = self.data.add_memory({constants.REWARD: 2})
        memories = [mem1, mem2]
    ...
    def test_append_working_memories(self):
        mem1 = self.data.add_memory({constants.MEMORY_DURATION: memory.INSTANT_MEMORY})
        working_memories = []
        new_memories = [mem1]
        ...
        memory.append_working_memories(working_memories, new_memories)
        self.assertEquals(1, len(working_memories))
        mem2 = self.data.add_memory({constants.MEMORY_DURATION: memory.INSTANT_MEMORY})
        mem3 = self.data.add_memory({constants.MEMORY_DURATION: memory.INSTANT_MEMORY})
    ...
    def test_prepare_expectation(self):
        mem1 = self.data.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY})
        mem2 = self.data.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY})
        imem1 = self.data.add_memory(
            {constants.MEMORY_DURATION: memory.INSTANT_MEMORY, constants.STATUS: constants.MATCHING,
             memory.CHILD_MEM: [5, 6, mem1[constants.MID], mem2[constants.MID]]})
        imem2 = self.data.add_memory(
            {constants.MEMORY_DURATION: memory.INSTANT_MEMORY, constants.STATUS: constants.MATCHING})
        smem1 = self.data.add_memory(
            {constants.MEMORY_DURATION: memory.SHORT_MEMORY, constants.STATUS: constants.MATCHING,
             memory.CHILD_MEM: [5, 6, imem1[constants.MID], imem2[constants.MID]]})
    ...
    def test_verify_slice_memory_match_result(self):
        fmem1 = self.data.add_memory({constants.STATUS: constants.MATCHED})
        fmem2 = self.data.add_memory({constants.STATUS: constants.MATCHED})
        fmem3 = self.data.add_memory({constants.STATUS: constants.MATCHING})
        smem1 = self.data.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY, constants.STATUS: constants.MATCHING,
                                          memory.CHILD_MEM: [fmem1[constants.MID], fmem2[constants.MID]]})
        smem2 = self.data.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY, constants.STATUS: constants.MATCHING,
