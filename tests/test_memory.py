...
class TestMemory(unittest.TestCase):
    ...
    def test_find_max_related_memory_success(self):
        ...
        mem2 = self.database.add_memory({constants.PARENT_MEM: [5, 6, mem1[constants.ID]]})
        mem3 = self.database.add_memory({constants.PARENT_MEM: [6, mem1[constants.ID], 8]})
    ...
    def test_find_max_related_memory_empty(self):
        mem1 = self.database.add_memory({constants.PARENT_MEM: [5, 6, 7]})
        mem2 = self.database.add_memory({constants.PARENT_MEM: [6, 7, 8]})
    ...
    def test_find_max_related_memory_limit(self):
        ...
        mem3 = self.database.add_memory({constants.PARENT_MEM: [5, 6, mem1[constants.ID], mem2[constants.ID]]})
        mem4 = self.database.add_memory({constants.PARENT_MEM: [6, mem1[constants.ID], mem2[constants.ID], 8]})
    ...
    def get_live_sub_memories_success(self):
        mem1 = self.database.add_memory()
        mem2 = self.database.add_memory()
        mem3 = self.database.add_memory({memory.CHILD_MEM: [mem1[constants.ID], mem2[constants.ID]]})
        child_memories1 = memory.get_live_sub_memories(mem3, memory.CHILD_MEM)
        self.assertEqual(2, len(child_memories1))

        mem4 = self.database.add_memory({memory.CHILD_MEM: [mem1[constants.ID], mem2[constants.ID], 9]})
        child_memories2 = memory.get_live_sub_memories(mem4, memory.CHILD_MEM)
        self.assertEqual(2, len(child_memories2))
        self.assertEqual(mem2[constants.ID], child_memories2[1][constants.ID])

        el5 = self.database.add_memory({memory.CHILD_MEM: [mem1[constants.ID], 9, mem2[constants.ID]]})
        child_memories3 = memory.get_live_sub_memories(el5, memory.CHILD_MEM)
        self.assertEqual(2, len(child_memories3))
        self.assertEqual(mem2[constants.ID], child_memories3[1][constants.ID])
    ...
    def test_get_memories(self):
        mem1 = self.database.add_memory({constants.STATUS: constants.MATCHED, constants.DURATION: 1, constants.REWARD: 0.5})
        mem2 = self.database.add_memory({constants.STATUS: constants.MATCHING, constants.DURATION: 1, constants.REWARD: 0.6})
        mem3 = self.database.add_memory({constants.STATUS: constants.EXPIRED, constants.DURATION: 3, constants.REWARD: 0.3})
    ...
    def test_find_max_related_memories(self):
        ...
        mem2 = self.database.add_memory()
        mem3 = memory.BASIC_MEMORY.copy()
        mem3.update({constants.ID: 3})
        mem4 = memory.BASIC_MEMORY.copy()
        mem4.update({constants.ID: 4})
        mem5 = self.database.add_memory({constants.PARENT_MEM: [mem1[constants.ID], mem2[constants.ID], mem3[constants.ID]]})
        mem6 = self.database.add_memory({constants.PARENT_MEM: [mem2[constants.ID], mem3[constants.ID], mem4[constants.ID]]})
        memories = [mem5, mem6]
        ...
        mem7 = self.database.add_memory({constants.PARENT_MEM: [mem2[constants.ID], mem3[constants.ID], mem4[constants.ID]]})
        ...
        self.assertEquals(mem2[constants.ID], related_memories2[0][constants.ID])
    ...
    def test_search_sub_memories(self):
        ...
        mem6 = self.database.add_memory({memory.CHILD_MEM: [mem4[constants.ID], mem2[constants.ID]]})
        memories = [mem3, mem6]
    ...
    def test_create_working_memory(self):
        ...
        mem1 = self.database.add_memory({constants.REWARD: 1})
        mem2 = self.database.add_memory({constants.REWARD: 2})
        memories = [mem1, mem2]
    ...
    def test_append_working_memories(self):
        mem1 = self.database.add_memory({constants.MEMORY_DURATION: memory.INSTANT_MEMORY})
        working_memories = []
        new_memories = [mem1]
        ...
        memory.append_working_memories(working_memories, new_memories)
        self.assertEquals(1, len(working_memories))
        mem2 = self.database.add_memory({constants.MEMORY_DURATION: memory.INSTANT_MEMORY})
        mem3 = self.database.add_memory({constants.MEMORY_DURATION: memory.INSTANT_MEMORY})
    ...
    def test_associate_slice_empty(self):
        ...
        mem0 = self.database.add_memory({constants.END_TIME: 1})
        ...
        mem3 = self.database.add_memory(
            {constants.END_TIME: now, constants.STATUS: constants.MATCHED,
             constants.PARENT_MEM: [5, 6, pmem1[constants.ID], pmem2[constants.ID]]})
        mem4 = self.database.add_memory(
            {constants.END_TIME: now, constants.STATUS: constants.MATCHED,
             constants.PARENT_MEM: [6, pmem1[constants.ID], pmem2[constants.ID], 8]})
    ...
    def test_prepare_expectation(self):
        mem1 = self.database.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY})
        mem2 = self.database.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY})
        imem1 = self.database.add_memory(
            {constants.MEMORY_DURATION: memory.INSTANT_MEMORY, constants.STATUS: constants.MATCHING,
             memory.CHILD_MEM: [5, 6, mem1[constants.ID], mem2[constants.ID]]})
        ...
        smem1 = self.database.add_memory(
            {constants.MEMORY_DURATION: memory.SHORT_MEMORY, constants.STATUS: constants.MATCHING,
             memory.CHILD_MEM: [5, 6, imem1[constants.ID], imem2[constants.ID]]})
    ...
    def test_check_expectation(self):
        ...
        imem1 = self.database.add_memory(
            {constants.MEMORY_DURATION: memory.INSTANT_MEMORY, constants.STATUS: constants.MATCHING,
             memory.CHILD_MEM: [5, 6, mem1[constants.ID], mem2[constants.ID]], constants.END_TIME: new_time})
        imem2 = self.database.add_memory(
            {constants.MEMORY_DURATION: memory.INSTANT_MEMORY, constants.STATUS: constants.MATCHING,
             memory.CHILD_MEM: [mem3[constants.ID]], constants.END_TIME: new_time})
        smem1 = self.database.add_memory(
            {constants.MEMORY_DURATION: memory.SHORT_MEMORY, constants.STATUS: constants.MATCHING,
             memory.CHILD_MEM: [5, 6, imem1[constants.ID], imem2[constants.ID]], constants.END_TIME: new_time})
    ...
    def test_verify_slice_memory_match_result(self):
        fmem1 = self.database.add_memory({constants.STATUS: constants.MATCHED})
        fmem2 = self.database.add_memory({constants.STATUS: constants.MATCHED})
        fmem3 = self.database.add_memory({constants.STATUS: constants.MATCHING})
        smem1 = self.database.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY, constants.STATUS: constants.MATCHING,
                                          memory.CHILD_MEM: [fmem1[constants.ID], fmem2[constants.ID]]})
        smem2 = self.database.add_memory({constants.MEMORY_DURATION: constants.SLICE_MEMORY, constants.STATUS: constants.MATCHING,
                                          memory.CHILD_MEM: [fmem1[constants.ID], fmem3[constants.ID]]})
        slice_memories = [smem1, smem2]
        slice_memory_children = {smem1[constants.ID]: [fmem1, fmem2], smem2[constants.ID]: [fmem1, fmem3]}
