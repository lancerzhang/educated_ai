...
def expect(expectation, parent_id, slice_expectations, total_matched_count, less_workload):
    ...
    #                 if mem:
    #                     now = time.time()
    #                     sub_expectation = mem.update(
    #                         {'id_time': now, 'status': NEW, START_TIME: now + interval - varience, END_TIME: now + interval + varience + 0.1, 'memory': {}, 'parent_id': parent_id,
    #                          'children': {}})
    ...
    #                     now = time.time()
    #                     sub_expectation = mem.update(
    #                         {'id_time': now, 'status': MATCHING, START_TIME: expectation[START_TIME], END_TIME: expectation[END_TIME], 'memory': {}, 'parent_id': parent_id})
    #                     expectation.children.push(sub_expectation)
    #                     slice_expectations.push(sub_expectation)
    #                 else:
    #                     print "forgot"
    ...
    #                 now = time.time()
    #                 sub_expectation = mem.update(
    #                     {'id_time': time.time(), 'status': NEW, START_TIME: now, END_TIME: now + 0.15, 'memory': {}, 'parent_id': parent_id, 'children': {}})
    #                 expectation.children.push(sub_expectation)
