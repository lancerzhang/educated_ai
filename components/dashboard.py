...
def log(memories, label):
    ...
    for x in memories:
        if x.live and x.status in [constants.MATCHING, constants.MATCHED] and x.active_end_time:
            ...
            active_time = 0
        m = [x.memory_type, x.feature_type, x.recall_count, int((time.time() - x.created_time) / 60), int(active_time)]
        data.append(m)
    data = np.array(data)
    field_type = 'type'
    ...
    df = pd.DataFrame(data, columns=[field_type, field_feature, field_recall, field_live_time, field_active_time])
