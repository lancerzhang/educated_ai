...
from components.recognizer import ImgShapeRecognizer
...
def match_block_seq(img_seq, move_seq, img, width, x_start, y_start):
    ...
    for i in range(1, len(img_seq)):
        ...
        if not ImgShapeRecognizer(block1).is_similar(block2):
            return False
        else:
...
def search_block_seq(img_seq, move_seq, img, size):
    ...
    for j in range(y):
        for i in range(x):
            ...
            if ImgShapeRecognizer(img_seq[0]).is_similar(block2):
